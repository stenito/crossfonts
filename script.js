...
const __djsf = (numberOfCharacters = 2, backgroundColor, textColor) => {
    if (document.querySelectorAll("[rel*=icon]").length < 1) {
        ...
        if (numberOfCharacters == 2 && words.length > 1) {
            iconText = words[0].substr(0, 1) + words[1].substr(0, 1);
        } else if (numberOfCharacters == 2 && words.length == 1) {
            iconText = words[0].substr(0, 2);
        } else {
            iconText = words[0].substr(0, 1);
        }
